...
def train_epoch(model, device, train_loader, criterion, optimizer):
    ...
    for batch in tqdm(train_loader, total=len(train_loader)):
        ...
        train_acc += (labels == (outputs > 0.5)).sum().cpu().item()
...
def test_epoch(model, device, test_loader, criterion):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, total=len(test_loader)):
            ...
            images_cnt += images.shape[0]
            test_acc += (labels == (outputs > 0.5)).sum().cpu().item()
    return test_loss / images_cnt, test_acc / images_cnt


# def get_emb(model, device, loader):
#     model.eval()
#     embs = []
#     with torch.no_grad():
#         for batch in tqdm(loader, total=len(test_loader)):
#             images = batch['photo'].to(device)
#             labels = batch['target'].to(device)
            
#             outputs = model(images).squeeze() #logits

#             embs.extend(outputs.detach().cpu().numpy().tolist())
#     return embs
...
def get_predictions(model, device, val_loader, add_sigmoid=False):
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, total=len(val_loader)):
            ...
            with torch.no_grad():
                if add_sigmoid:
                    outputs = torch.sigmoid(model(images).logits.squeeze())
                else:
                    outputs = model(images).logits.squeeze()
